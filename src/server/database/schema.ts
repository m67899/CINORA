...
export const comments = pgTable("comments", { id: id(), userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }), titleId: uuid("title_id").references(() => titles.id, { onDelete: "cascade" }), reelId: uuid("reel_id"), body: text("body").notNull(), createdAt: createdAt(), updatedAt: updatedAt() });
